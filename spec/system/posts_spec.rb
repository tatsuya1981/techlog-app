    ...
    describe 'ログ投稿機能の検証' do
        ...
        context 'ログインしている場合' do
            ...
            it 'ログインページへリダイレクトしない' do
                expect(current_path).not_to eq('/users/sing_in')
            end
    ...
    describe 'ログ削除機能の検証' do
        context '投稿したユーザーでログインしている場合' do
            ...
            it '削除ボタンを押すと削除できる' do
                expect do
                    click_button '削除'
                end.to change(Post, :count).by(-1) # 削除ボタンをクリックするとPostが１つ減る
                
                # リダイレクト後の画面確認
                expect(current_path).to eq('/')
                expect(page).to have_content('投稿が削除されました') # フラッシュメッセージ表示
                expect(page).not_to have_link("/posts/#{@post.id}") # 削除した投稿が存在しない
            end
        end
